# -*- coding: utf-8 -*-
"""
Created on Fri Nov 11 07:58:36 2016
...
train_ids = pd.read_csv(MODELDIR+oof_list[0], usecols=['Id'], squeeze=True).values
test_ids = pd.read_csv(MODELDIR+preds_list[0], usecols=['Id'], squeeze=True).values
...
train_avg = train.mean(1)
test_avg = test.mean(1)
train_gmean = gmean(train, 1)
test_gmean = gmean(test, 1)

best_proba, best_mcc, _ = eval_mcc2(y, train_avg)
print("Out of fold (predicted LB) CV mcc: {}".format(best_mcc))

nfolds=1
out_from_probs(train_ids, train_avg, test_ids, test_avg, best_proba,
               nfolds, best_mcc, 'avg_ensemble2')

best_proba, best_mcc, _ = eval_mcc2(y, train_gmean)
print("Out of fold (predicted LB) CV mcc: {}".format(best_mcc))

nfolds=1
out_from_probs(train_ids, train_gmean, test_ids, test_gmean, best_proba,
               nfolds, best_mcc, 'gmean_ensemble2')
...
#    return
#
#eval_xgb(train, y, test, train_ids, test_ids, nfolds=20, random_state=9)
#
#
...
#clf = MLPClassifier(hidden_layer_sizes=(400,100,20), random_state=0,
#                    tol=.00005, verbose=True)
#eval_clf(clf, train, y, test, train_ids, test_ids, 'MLPensemble1', 10, 16)